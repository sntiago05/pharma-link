...
export function admin_controller(){
    ...
    form_disable_user.addEventListener("submit", async (e)=>{
        ...
        const userExists = users.some((user) => user.email === disableData.user_email);
        ...
        if (!disableData.user_email) {
            ...
            getElementById("disable_message").innerHTML = `<span class="border border-red-600 bg-red-400 text-red-700">The email does not exist in the users array!</span>`;
            setTimeout(()=>{getElementById("disable_message").innerHTML = ``;}, 1500);
        } else {
