...
CREATE TABLE pharmacies (
  ...
  parent_pharmacy_id INTEGER REFERENCES pharmacies(id)
);
...
CREATE TABLE patients (
  id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  user_id INTEGER UNIQUE REFERENCES users(id), eps_id INTEGER NOT NULL REFERENCES eps(id),
  document VARCHAR(30) NOT NULL UNIQUE, phone VARCHAR(20),
  full_name VARCHAR(150), email VARCHAR(150)
);
